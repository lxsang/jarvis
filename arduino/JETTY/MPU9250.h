...
public:
    ...
    int initialize(WireType &w = Wire)
    {
        wire = &w;
    ...
    void setAccBias(uint8_t i, float v)
    {
        if (i < 3)
            accelBias[i] = v;
    }
    void setGyroBias(uint8_t i, float v)
    {
        if (i < 3)
            gyroBias[i] = v;
    }
...
private:
    ...
    void updateMag()
    {
        ...
        m[0] = (float)(magCount[0] * mRes * magCalibration[0]); // get actual magnetometer value, this depends on scale being set
